  ...
  Future<void> _reserverCreneau(int creneauId, BuildContext context) async {
    try {
      ...
      try {
        ...
        final result = await _apiService.createReservation(userId, creneauId);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('✅ ${result['message'] ?? 'Créneau réservé avec succès'}'),
        ...
        // Mettre à jour la disponibilité du créneau localement
        setState(() {
          _futureCreneaux = _futureCreneaux.then((creneaux) {
            final index = creneaux.indexWhere((c) => c.id == creneauId);
            if (index != -1) {
              creneaux[index] = creneaux[index].copyWith(disponibilite: false);
            }
            return creneaux;
          });
        });
